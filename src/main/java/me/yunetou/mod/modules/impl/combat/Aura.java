    ...
    public class Aura
        ...
        private Entity getTarget() {
            ...
            for (Entity entity : Aura.mc.world.loadedEntityList) {
                if (!(this.players.getValue() && entity instanceof EntityPlayer || this.animals.getValue() && EntityUtil.isPassive(entity) || this.neutrals.getValue() && EntityUtil.isNeutralMob(entity) || this.hostiles.getValue() && EntityUtil.isMobAggressive(entity) || this.hostiles.getValue() && this.onlyGhasts.getValue() && entity instanceof EntityGhast || this.others.getValue() && EntityUtil.isVehicle(entity)) && (!this.projectiles.getValue() || !EntityUtil.isProjectile(entity)) || EntityUtil.invalid(entity, this.range.getValue()) || !Aura.mc.player.canEntityBeSeen(entity) && !EntityUtil.isFeetVisible(entity) && Aura.mc.player.getDistanceSq(entity) > MathUtil.square(this.wallRange.getValue()) || this.fovCheck.getValue() && !this.isInFov(entity, this.angle.getValue().intValue()))
                    continue;
                if (target == null) {
                    target = entity;
                ...
                if (this.targetMode.getValue() != TargetMode.DISTANCE || !((double) Aura.mc.player.getDistance(entity) < distance))
                    continue;
                target = entity;
                distance = Aura.mc.player.getDistance(entity);
                maxHealth = EntityUtil.getHealth(entity);
