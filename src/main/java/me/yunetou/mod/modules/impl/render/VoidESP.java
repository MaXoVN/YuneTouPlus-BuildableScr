...
import me.yunetou.api.util.render.RenderUtil3;
...
public class VoidESP
    ...
    private void drawVoidESP(BlockPos pos, Color color, Color secondColor, boolean outline, boolean box, int boxAlpha, double height, boolean cross) {
        if (box) {
            RenderUtil3.drawBox(pos, new Color(color.getRed(), color.getGreen(), color.getBlue(), boxAlpha), height, false, false, 0);
        }
        if (outline) {
            RenderUtil3.drawBlockOutline(pos, color, 0.8f, true, height, false, false, 0, false);
        }
        if (cross) {
            RenderUtil3.drawBlockWireframe(pos, color, 0.8f, height, true);
        }
    }
