    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (this.setting.open) {
            for (Object o : this.setting.getValue().getClass().getEnumConstants()) {
                ...
                Managers.TEXT.drawStringWithShadow((Object)(this.setting.getCurrentEnumName().equals(s) ? ChatFormatting.WHITE : ChatFormatting.GRAY) + s,
                        (float)this.width / 2.0f - (float)Managers.TEXT.getStringWidth(s) / 2.0f + 2.0f + this.x,
                        (float)(y += 12) + 6.0f - (float)EnumButton.mc.fontRenderer.FONT_HEIGHT / 2.0f + 3.5f, -1);
            }
        }
    }
