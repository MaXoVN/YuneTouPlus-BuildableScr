...
extends Button {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (dotgod) {
            ...
            // Default rendering with gradient on hover
            if (this.isHovering(mouseX, mouseY) && Mouse.isButtonDown(0)) {
                RenderUtil.drawHGradientRect(
                        ...
                        ColorUtil.pulseColor(new Color(ClickGui.INSTANCE.color.getValue().getRed(), ClickGui.INSTANCE.color.getValue().getGreen(), ClickGui.INSTANCE.color.getValue().getBlue(), 200), 50, 1).getRGB(),
                        ColorUtil.pulseColor(new Color(ClickGui.INSTANCE.color.getValue().getRed(), ClickGui.INSTANCE.color.getValue().getGreen(), ClickGui.INSTANCE.color.getValue().getBlue(), 200), 50, 1000).getRGB()
                );
            } else {
                RenderUtil.drawRect(
                        this.x,
