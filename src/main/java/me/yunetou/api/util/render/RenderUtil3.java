...
public class RenderUtil3
    ...
    public static void drawBoxESP(BlockPos pos, Color color, boolean secondC, Color secondColor, float lineWidth, boolean outline, boolean box, int boxAlpha, boolean air) {
        if (box) {
            RenderUtil3.drawBox(pos, new Color(color.getRed(), color.getGreen(), color.getBlue(), boxAlpha));
        }
        if (outline) {
            RenderUtil3.drawBlockOutline(pos, secondC ? secondColor : color, lineWidth, air);
        }
    }
    ...
    public static void drawFadingSide(BlockPos pos, EnumFacing face, Color startColor, Color endColor, double height) {
        ...
        if (face == EnumFacing.EAST || face == EnumFacing.WEST || face == EnumFacing.NORTH || face == EnumFacing.SOUTH) {
            RenderUtil3.buildPosColor(builder, red, green, blue, alpha, red2, green2, blue2, alpha2, x1, y1, z1, x2, y2, z2);
        } else if (face == EnumFacing.UP) {
            RenderUtil3.buildPosColor(builder, red2, green2, blue2, alpha2, x1, y1, z1, x2, y2, z2);
        } else if (face == EnumFacing.DOWN) {
            RenderUtil3.buildPosColor(builder, red, green, blue, alpha, x1, y1, z1, x2, y2, z2);
        }
        tessellator.draw();
        GlStateManager.depthMask(true);
        GlStateManager.disableBlend();
    ...
    private static void buildPosColor(BufferBuilder builder, float red2, float green2, float blue2, float alpha2, double x1, double y1, double z1, double x2, double y2, double z2) {
        RenderUtil3.buildPosColor(builder, red2, green2, blue2, alpha2, red2, green2, blue2, alpha2, x1, y1, z1, x2, y2, z2);
    }
    ...
    public static void drawBlockOutline(BlockPos pos, Color color, float linewidth, boolean air) {
        ...
        if ((air || iblockstate.getMaterial() != Material.AIR) && RenderUtil.mc.world.getWorldBorder().contains(pos)) {
            ...
            RenderUtil3.drawBlockOutline(iblockstate.getSelectedBoundingBox(RenderUtil.mc.world, pos).grow(0.002f).offset(-interp.x, -interp.y, -interp.z), color, linewidth);
        }
    }
